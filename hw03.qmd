...
cat("95% confidence interval:", k_ci, "\n")
...
print("beta's point estimate, 95% confidence interval :")
print(beta1_hat)
print(beta1_ci)
print("very similar to the result of 1")
...
print(p_resid_eta)
print("The model results in a graph with no distinct curvilinear patterns, and residuals randomly scattered around zero without clustering on one side.")
...
print("logRA, logRS point estimate, 95% confidence interval :")
print(coef_logs)
print(ci_logs)
...
cat("logRS coefficient + logRA coefficient:", coef_logs[["logRS"]] + coef_logs[["logRA"]], "\n")
print("Even if log(RS) and log(RA) are input separately, the model is almost the same as the model with only log(RS/RA) input.")
...
k_hat = popt[0]
print("k's point estimate:", k_hat)
...
print("95% confidence interval:", k_ci)
...
print(
    "The model results in a graph with no distinct curvilinear patterns, and residuals randomly scattered around zero without clustering on one side."
)
...
print("logRA, logRS 's point estimates, 95% confidence intervals:")
print(coef_logs)
print(ci_logs)
...
모형의 복잡도가 올라갔음에도 가중 MAE, RMSE 모두 감소했다. 특히 AIC가 유의미하게 감소했다. 
따라서 final model이 더 좋은 성능을 보인다. 
...
if (!is.null(nb_step_model)) {
  ...
  nb_coef <- coef(nb_step_model)
  nb_se <- sqrt(diag(vcov(nb_step_model)))
  nb_coef_table <- data.frame(
    Estimate = nb_coef,
    Std_Error = nb_se,
    z_value = nb_coef / nb_se,
    p_value = 2 * pnorm(abs(nb_coef / nb_se), lower.tail = FALSE)
  )
  print(nb_coef_table)
  cat("Estimated theta:", nb_step_model$theta, "\n")
  cat("SE(theta):", nb_step_model$SE.theta, "\n")
  if (is.finite(nb_step_model$theta) && nb_step_model$theta > 1000) {
...
서로 다른 모형 계열을 비교할 때는 AIC보다는 W_MAE 등의 예측오차 지표를 더 중요하게 보는 것이 적절하다.
